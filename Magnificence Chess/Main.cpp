...
void guiInterface()
{
	...
	while (getline(cin, recievedCommand))
	{
		...
		if (CONSOLEDEBUG) //Debug commands, not used for interface!
		{
			...
			{
				timer = clock();
				unsigned long perftNumber = Test::perft(stoi(splitCommand[1]), &board, color);
			...
			{
				timer = clock();
				string result = Test::perftDivide(stoi(splitCommand[1]), &board, color);
				duration = (clock() - timer) / (double)CLOCKS_PER_SEC;
				cout << result << endl;
			}
			...
			else if ((splitCommand[0] == "moves" || splitCommand[0] == "lmov") && splitCommand.size() == 2)
			{
				...
				{
					vector<u32> moves = board.WhiteLegalMoves();
				...
				{
					vector<u32> moves = board.BlackLegalMoves();
					cout << "Move count: " << moves.size() << endl;
					for (size_t i = 0; i < moves.size(); i++)
					{
						cout << IO::convertMoveToAlg(moves[i]) << " [" <<
